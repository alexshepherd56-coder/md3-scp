...
console.log('[FlagUI] Script loaded - version 2024-12-26-v2');
...
class FlagUI {
  ...
  initialize(flagModule) {
    ...
    if (!this.flagModule) {
      console.error('[FlagUI] ERROR: flagModule is null/undefined!');
      return;
    }

    console.log('[FlagUI] FlagModule reference set, flags loaded:', this.flagModule.getFlagCount());

    // Listen to flag events
    this.setupEventListeners();

    // Initial UI update
    console.log('[FlagUI] Calling updateAllUI...');
    this.updateAllUI();
  ...
  setupEventListeners() {
    ...
    // Listen for navigation/content changes to update badges on newly visible cards
    this.eventBus.on('navigation:content-changed', (data) => {
      console.log('[FlagUI] Content changed, updating badges:', data);
  ...
  /**
   * Update flagged question count badge on a case card
   * @param {string} caseId - The case ID
   * @param {HTMLElement} [cardElement] - Optional specific card element to update
   */
  updateCaseFlaggedQuestionsBadge(caseId, cardElement = null) {
    // If no specific card provided, update ALL cards matching this caseId
    const cards = cardElement
      ? [cardElement]
      : document.querySelectorAll(`a.case-card[href*="case${caseId}"]`);
  ...
  updateAllFlaggedQuestionsBadges() {
    const caseCards = document.querySelectorAll('.case-card');
    const allFlags = this.flagModule.getAllFlags();

    console.log('[FlagUI] updateAllFlaggedQuestionsBadges - found', caseCards.length, 'cards,', allFlags.length, 'total flags');
    ...
    caseCards.forEach(card => {
      ...
      if (match) {
        const caseId = match[1];
        const flagsForCase = this.flagModule.getFlagsForCase(caseId);
        if (flagsForCase.length > 0) {
          console.log('[FlagUI] Case', caseId, 'has', flagsForCase.length, 'flags');
        }
        this.updateCaseFlaggedQuestionsBadge(caseId, card);
      }
    });
  }
